...
// SPDX-License-Identifier: AGPL-3.0-only
  ...
  const __dfcPortraitExhausted = new Set(); // Sources that failed enough times to use the glyph.
  ...
  window.dfcPortraitLoad = function(img) {
    ...
    // actually decoded to a portrait. DF's portrait textures are 96x96; the old server could
    // return a successful 32x32 map-sprite response here and the client would falsely remember it
    // as native. Display size is controlled by CSS, so validating the source dimensions is safe.
  ...
  // glyph. Parity fix: the SHEET portrait requests its native bust once per unit per session.
  // The server queues that request for DF's ordinary update lifecycle; it never recursively calls
  // native screen logic. This applies to every race -- authored-portrait animals excluded (their crop IS the
