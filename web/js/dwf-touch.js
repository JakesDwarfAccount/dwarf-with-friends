...
// ---- Touch-gesture layer for the map canvas, mapped onto the SAME camera and click primitives. ----
// Feature detection, never UA sniffing: every POINTER listener early-returns unless pointerType is "touch".
...
(function () {
  ...
  // Fingers moving UP (dy negative) = ascend (+z), mirroring plain-wheel-up = +z.
  ...
  // handle*(pt) takes {pointerId, clientX, clientY} and returns TRUE when the event was consumed, so the
  // browser layer preventDefaults it; FALSE means it must flow through to the existing handlers untouched.
  ...
  function bindBrowser() {
    ...
    const hooks = {
      ...
      zStep: dz => { try { window.DFTouchNav && window.DFTouchNav.zStep(dz); }
        catch { /* pointer-move z swipe retries on the next gesture sample */ } },
      zoomToPx: px => { try { window.DFTouchNav && window.DFTouchNav.zoomToPx(px); }
      ...
      tap: (x, y) => {
        try {
          ...
          for (const type of ["pointerdown", "pointerup"]) {
            ...
            try { ev.__dfTouchSynthetic = true; } catch (err) { DwfErr.report("touch.synthetic-marker", err); }
            view.dispatchEvent(ev);
          }
        } catch (err) { DwfErr.report("touch.synthetic-dispatch", err); }
      },
    };
    ...
    // Browsers that OVERLAY the keyboard shrink visualViewport but not the layout viewport, so bottom-
    // anchored chrome offsets itself by this var; browsers that resize the layout viewport yield inset 0.
    ...
    try { window.DFTouch = { _controller: ctrl }; } catch (err) { DwfErr.report("touch.export", err); }
  }
