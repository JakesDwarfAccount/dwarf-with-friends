...
// The html here is a static, trusted blurb (never user input), so it is embedded as-is.
function info(html) {
  return `<details class="setup-info"><summary>What is this?</summary><div class="setup-info-body">${html}</div></details>`;
}
// One blurb per step, in the wizard's voice.
const INFO = {
  dfhack: `DFHack is the community modding engine that Dwarf Fortress mods run on, and Dwarf With Friends is built as a DFHack plugin — so the game needs it before the mod can load. Setup downloads DFHack straight from its official GitHub releases, pinned to the exact version 53.16-r1 and hash-verified before it is accepted. It installs into your Dwarf Fortress folder exactly the way a manual DFHack install would; nothing goes anywhere else.`,
...
function render(s) {
  ...
  if (x.df.ok && x.dfhack.missing) {
    hackBody = `DFHack is not installed in this Dwarf Fortress folder. DWF requires exactly DFHack ${esc(x.dfhack.version?.required || "53.16-r1")}.`;
    hackAction = button("install-dfhack", "Install DFHack for me");
  } else if (x.dfhack.wrongVersion) {
    hackBody = `<strong>Version mismatch:</strong> DFHack ${esc(x.dfhack.version.version)} is installed, but this mod was built for exactly 53.16-r1 and will NOT load on any other build. Replace this DFHack with <a href="https://github.com/DFHack/dfhack/releases/tag/53.16-r1" target="_blank" rel="noopener">53.16-r1</a> (delete the old <code>hack</code> folder first), then run setup again.`;
    ...
    hackBody = `DFHack is installed, but its version could not be read. DWF only works with exactly DFHack 53.16-r1 — on any other build the mod does not load. Install the known-good version, or proceed if you are sure this is 53.16-r1.`;
    hackAction = button("install-dfhack", "Install DFHack 53.16-r1 for me") + button("proceed-wrong-dfhack", "Proceed with this DFHack");
