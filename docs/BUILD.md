...
This plugin builds as an external plugin inside a DFHack 53.16-r1 source tree. The
repository's target is named `dfcapture_public`, with output name `dwf`.

## Requirements

- A DFHack 53.16-r1 source checkout, including its normal build prerequisites.
...
- **Linux:** GCC and DFHack's normal Linux prerequisites (cmake, ninja/make, zlib, SDL2
  ...
  of GDI+. Use a compiler supported by the pinned DFHack version, such as GCC 15.
  GCC 16 is rejected by that DFHack version's compiler check.
...
add `add_subdirectory(multi-dwarf)` once if it is absent.
...
Close Dwarf Fortress first. Copy the Windows DLL to `<Dwarf Fortress>/hack/plugins/dwf.plug.dll`,
or the Linux shared object to `<Dwarf Fortress>/hack/plugins/dwf.plug.so` with mode 0755. Copy `web/` to
...
When switching source checkouts in an existing build directory, add `--clean-first` to the build command. Never replace a loaded plugin.
