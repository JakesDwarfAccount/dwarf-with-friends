...
> the `dfcap_auth` cookie. Renaming those would break existing installs, so it is deferred.
...
| Guarded native actions | `dfcapture-hostwrites.json`, Lua `trade_*` and `justice_*` flags. These flags are not settable over HTTP. | Off unless the action’s flag is literal `true`. | Controls specific native trade and justice actions. Unsupported actions remain unavailable; changing a flag does not make an unsupported operation safe. |
