...
public:
  ...
  // Pre-routing gate (backported subset of upstream set_pre_routing_handler): runs for EVERY
  // request before the file handler / route dispatch. Returning true means the handler fully
  // formed `res` and routing must stop (short-circuit); false lets normal routing proceed.
  // Used by dwf's join-security auth gate (http_server.cpp) so one place guards every
  // protected route -- no per-handler wrapping to forget.
...
inline bool Server::routing(Request &req, Response &res, Stream &strm) {
  // Pre-routing gate (dwf join-security auth). Runs before everything, incl. the static
  // file handler, so a protected request is rejected no matter which handler would serve it.
