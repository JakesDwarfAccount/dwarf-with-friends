...
void set_diagnostics_verbose(bool on) { g_diag_verbose.store(on, std::memory_order_relaxed); }
...
std::atomic<long long> g_phase_enter_ms{0};
std::atomic<bool> g_phase_inside{false};
} // namespace
...
void diag_phase_enter(const char* name) {
    g_phase_name.store(name, std::memory_order_relaxed);
    g_phase_enter_ms.store(diag_steady_ms(), std::memory_order_relaxed);
    g_phase_seq.fetch_add(1, std::memory_order_relaxed);
    g_phase_inside.store(true, std::memory_order_relaxed);
}

void diag_phase_exit() {
    g_phase_inside.store(false, std::memory_order_relaxed);
}

PhaseSnapshot diag_phase_snapshot() {
    PhaseSnapshot s;
    s.name = g_phase_name.load(std::memory_order_relaxed);
    s.seq = g_phase_seq.load(std::memory_order_relaxed);
    s.entered_ms = g_phase_enter_ms.load(std::memory_order_relaxed);
    s.inside = g_phase_inside.load(std::memory_order_relaxed);
