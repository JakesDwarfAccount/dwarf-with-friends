...
void register_work_order_routes(httplib::Server& server) {
    ...
    auto order_create_handler = [](const httplib::Request& req, httplib::Response& res) {
        ...
        attrib_note_world(world ? world->cur_savegame.save_dir : std::string());
        for (int32_t oid : created_ids)
    ...
    auto order_cond_item_edit_handler = [](const httplib::Request& req, httplib::Response& res) {
        ...
        int value = 0;
        if (!req.has_param("compare") || !query_int(req, "value", value)) {
            text_error(res, 400, "missing compare/value");
            return;
        }
        std::string compare = req.get_param_value("compare");
        std::string item = req.has_param("item") ? req.get_param_value("item") : "";
        std::string material = req.has_param("material") ? req.get_param_value("material") : "";
        std::string adjective = req.has_param("adjective") ? req.get_param_value("adjective") : "";

        std::string err;
        if (!edit_item_condition_via_lua(id, idx, compare, value, item, material, adjective,
                                         &err)) {
